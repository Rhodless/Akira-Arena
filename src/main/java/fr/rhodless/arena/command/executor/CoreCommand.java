...
public class CoreCommand extends org.bukkit.command.Command {
    ...
    public CoreCommand(CommandHandler coreCommandHandler, Method method, Object instance, Command command, String usage, List<String> aliases) {
        super(command.names()[0], command.description(), usage, aliases);
        this.handler = coreCommandHandler;
    ...
    public boolean execute(CommandSender commandSender, String alias, String[] args) {
        ...
        if (commandSender instanceof Player) {
            if (!commandSender.hasPermission(command.permission()) && !command.permission().equals("none")) {
                commandSender.sendMessage(CC.prefix(handler.getPermissionMessage()));
                return false;
            }
        }
            ...
            for (int index = 1; index < method.getParameterTypes().length; index++) {
                ...
                if (args.length == 0) {
                        ...
                        for (Annotation annotation : annotationArray) {
                            if (annotation instanceof Param) {
                                ...
                                if (param.baseValue().equalsIgnoreCase("")) bool = true;
                                    ...
                                    parameters.add(handler.getTypesMap().get(method.getParameterTypes()[i]).transform(commandSender, param.baseValue()));
                            }
                        }
                        i++;
                    }
                    if (bool) {
                        ...
                        if (commandSender instanceof Player)
                            commandSender.sendMessage(CC.prefix("&cMerci d'utiliser /" + (alias + " " + usage)));
                        return false;
                    }
                } else {
                    if (args.length <= index - 1 || args[index - 1] == null || args[index - 1].equals("")) {
                        ...
                        if (param.wildcard()) {
                            parameters.add(handler.getTypesMap().get(method.getParameterTypes()[index]).transform(commandSender, param.baseValue()));
                            break;
                        } else {
                            parameters.add(handler.getTypesMap().get(method.getParameterTypes()[index]).transform(commandSender, param.baseValue()));
                        }
                        ...
                        parameters.add(handler.getTypesMap().get(method.getParameterTypes()[index]).transform(commandSender, sb.toString()));
                        break;
                    } else
                        parameters.add(handler.getTypesMap().get(method.getParameterTypes()[index]).transform(commandSender, args[index - 1]));
                }
            }
        }
        ...
        if (command.async()) ForkJoinPool.commonPool().
                execute(() ->
                {
                    try {
    ...
    public List<String> tabComplete(CommandSender sender, String alias, String[] args) {
        ...
        PType<?> pType = handler.getTypesMap().get(method.getParameterTypes()[args.length]);
